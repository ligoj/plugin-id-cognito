# plugin-id-cognito UI

Vue sources for the Ligoj "id-cognito" tool-level plugin. Augments `plugin-id`
with AWS Cognito-specific labels and subscription row actions (activity export).
...
ui/
├── package.json
├── vite.config.js            # library build → ../src/main/resources/.../webjars/id-cognito/vue/
...
└── src/
    ...
    └── i18n/{en,fr}.js       # AWS Cognito-specific parameter labels
```
...
```sh
npm install
npm run dev        # standalone dev server on :5318; proxies REST to :8080
npm run build      # writes ../src/main/resources/META-INF/resources/webjars/id-cognito/vue/index.js
```
...
For AWS Cognito subscriptions (`service:id:ldap:*`) that lookup resolves to
